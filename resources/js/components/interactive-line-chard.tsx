...
function HoverPopup({ data, position, visible, config }: HoverPopupProps) {
    ...
    return (
        <div
            className="pointer-events-none absolute z-50 rounded-lg border border-gray-200 bg-white p-3 shadow-lg transition-opacity duration-200"
            style={{
                left: position.x + 10,
                top: position.y - 10,
        ...
        >
            <div className="space-y-2">
                <div className="flex items-center gap-2">
                    <div className="h-3 w-3 rounded-full" style={{ backgroundColor: config.color || '#3b82f6' }}></div>
                ...
                {data.details && <div className="max-w-48 text-sm text-gray-700">{data.details}</div>}
            </div>
            <div className="absolute top-full left-4 h-0 w-0 border-t-4 border-r-4 border-l-4 border-t-gray-200 border-r-transparent border-l-transparent"></div>
        </div>
    );
}
...
export function InteractiveLineChart({ data = [], config = {}, onPointClick, onDataFilter, loading = false, error }: InteractiveLineChartProps) {
    const {
        valueLabel = 'Value',
        dateLabel = 'Date',
        showSearch = true,
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-4">
                ...
                <Button variant={animationEnabled ? 'default' : 'outline'} onClick={() => setAnimationEnabled(!animationEnabled)} size="sm">
                    Animation
                </Button>
                {selectedPoint && (
